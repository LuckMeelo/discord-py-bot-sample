...
class CommandEventsCog(commands.Cog):
    ...
    async def on_command_completion(self, context: commands.Context) -> None:
        ...
        if context.guild is not None:
            self.bot.logger.info(f"Executed {executed_command} command in {context.guild.name} (ID: {context.guild.id}) by {context.author} (ID: {context.author.id})")
        else:
            self.bot.logger.info(f"Executed {executed_command} command by {context.author} (ID: {context.author.id}) in DMs")
    ...
    async def on_command_error(self, context: commands.Context, error) -> None:
        ...
        if isinstance(error, commands.CommandNotFound):
            self.bot.logger.warning(f"Command not found: {context.message.content} by {context.author} (ID: {context.author.id})")
